...
churn_cleaned <- churn_df %>%
  ...
  dplyr::mutate(across(c(
    ...
    age_with_company, avg_transaction_value, days_since_last_login
  ),as.numeric)) %>%
...
### Random Forrest Workflow with upsampling with all predictors
...
perf_table <- performance_RF %>% 
  ...
  mutate(performance_RF_up_all$.estimate) %>% 
...
column_names <- c("Metric","RF","RF_up","RF_up_all","RF_down","RF_rose","RF_boxcox",
...
CM_RF_Up_all <- CM_builder(predictions_RF_up_all, "churn")
...
ggarrange(CM_RF, CM_RF_Up, CM_RF_Up_all, CM_RF_Down, CM_RF_Rose, CM_RF_boxcox,
          CM_xg, CM_xg_Up, CM_xg_Down, CM_xg_Rose, CM_xg_boxcox,
          ncol = 5, nrow = 2, 
          labels = c("RF","RF_up","RF_up_all", "RF_down","RF_rose","RF_boxcox",
                  "XG","XG_up","XG_down","XG_rose","XG_boxcox"))
